...
class Check(QWidget):
        ...
        for i, row in enumerate(self.x):
            for j, elem in enumerate(row):
                if j == 4:
                    y = elem.split('-')
                    a, b, c = y[0], y[1], y[2]
                    self.d = datetime.datetime(year=a, month=b, day=c)
                    print(self.d)
                    if self.d < date.today():
                        self.result.append(self.cur.execute("""SELECT * FROM product""").fetchall())
        print(self.result)
        if self.result:
    ...
    def back(self):
        for i, row in enumerate(self.result):
            for j, elem in enumerate(row):
                print(i, j)
                print(QTableWidgetItem(str(elem)))
